...
class GameContainer extends Component {
  ...
  rendorGameContainer = () => {
    const {newRandomImage, imagesList, activeTab} = this.state
    const {imageUrl} = newRandomImage
    const filteredNewList = imagesList.filter(
      eachNewList => eachNewList.category === activeTab,
    )

    return (
      <>
        <div>
          <img src={imageUrl} alt="match" className="random-img-style" />
        </div>
        <div>
        ...
        <div className="all-image-card">
          <ul className="all-images">
            {filteredNewList.map(eachImoji => (
              <EmojiItems
                eachImoji={eachImoji}
                key={eachImoji.id}
                changeImage={this.onChangingImage}
              />
            ))}
          </ul>
        </div>
    ...
    return (
      <div className="main-cont">
        ...
        {isMatched ? (
          ...
          <YourScore count={count} onPlayAgain={this.onRestart} />
        )}
      </div>
    )
  }
